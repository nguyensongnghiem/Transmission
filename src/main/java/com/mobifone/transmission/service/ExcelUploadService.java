...
public class ExcelUploadService {
    ...
    @Autowired
    private static IFoContractRepository foContractRepository;
    @Autowired
    private static ISiteRepository siteRepository;
    public static boolean isValidExcelFile(MultipartFile file){
        return Objects.equals(file.getContentType(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" );
    }
    public static List<HiredFoLine> getHiredFoDataFromExcel(InputStream inputStream){
        List<HiredFoLine> hiredFoLines = new ArrayList<>();
        try {
            XSSFWorkbook workbook = new XSSFWorkbook(inputStream);
            XSSFSheet sheet = workbook.getSheet("hired_fo");
            int rowIndex =0;
            for (Row row : sheet){
                if (rowIndex ==0){
                    rowIndex++;
                ...
                while (cellIterator.hasNext()){
                    ...
                    switch (cellIndex){
                        case 0 -> hiredFoLine.setFoContract(foContractRepository.findByContractName(cell.getStringCellValue()));
